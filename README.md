...
Selected previews recheck the full saved source identity on one local handle and then read only the persisted candidate’s exact byte range. PNG, JPEG, GIF, and PDF candidate discovery additionally use method-specific fixed source windows with mandatory legacy parity; filesystem metadata, legacy compatibility discovery, other carvers, recovery, and exports retain their full-buffer compatibility paths. Exports intentionally retain a stricter full-source re-derivation and manifest-comparison path. The [source-access architecture](docs/source-access-architecture-v1.md), [JPEG windowed-discovery contract](docs/jpeg-window-discovery-v1.md), [GIF windowed-discovery contract](docs/gif-window-discovery-v1.md), and [PDF windowed-discovery contract](docs/pdf-window-discovery-v1.md) describe this staged design and its remaining time-of-check/time-of-use boundary.
