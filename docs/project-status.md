...
| PNG, JPEG, GIF, PDF, and ZIP/Open XML discovery | PNG, JPEG, GIF, PDF, and ZIP/Open XML structural discovery use fixed 1 MiB primary source windows, method-specific signature overlap and ownership, bounded file-backed validation, and legacy-candidate parity enforcement. JPEG retains the existing 128 MiB structural cap; GIF, PDF, and ZIP/Open XML retain their existing 64 MiB structural caps. | The full source is still buffered for filesystem metadata, legacy compatibility discovery, AVI/MP4/MOV carving, recovery, and exports; this is not a complete streaming-scan or parser-level cancellation claim. |
...
| Confidential reporting | GitHub private vulnerability reporting is enabled and the security policy directs confidential reports to the repository advisory form. | A reporting route is not a security certification, response-time commitment, signed artifact, or release authorization. |
...
The current strict interim assessment is **86/100 for local product and portfolio quality**, **91/100 for public repository quality**, and **64/100 for public-release readiness**. The product/portfolio and repository scores recognize the truthful scan and candidate-evidence guidance, bounded PNG/JPEG/GIF/PDF/ZIP/Open XML discovery, forensic safety controls, deterministic verification depth, a synthetic acceptance/refusal corpus, method-stage cancellation parity controls, protected four-context branch governance, hosted Linux/Windows/macOS ARM64/CodeQL evidence, native Windows installer install/uninstall acceptance, and generated SBOM review evidence. Release readiness remains deliberately lower because only five discovery methods are windowed while most discovery, recovery, and export paths remain full-buffer compatibility paths; parser loops still have no interruption/progress controls; macOS packaging/manual-acceptance and signing evidence are absent; the SBOM is not an attestation; and no semantic version or release provenance exists.[6] [13]
...
The completed PNG, JPEG, GIF, PDF, and ZIP/Open XML increments now have method-specific fixed-window discovery contracts, primary-window ownership, boundary/refusal controls, completed-window cancellation, and legacy candidate parity. JPEG retains the legacy absolute 128 MiB structural cap while scanning entropy data through bounded sequential file reads; GIF retains the legacy absolute 64 MiB cap while scanning its supported block stream through bounded sequential file reads; PDF retains the legacy absolute 64 MiB cap while scanning its supported header, EOF, `startxref`, and `xref` checks through bounded source reads; ZIP/Open XML retains the legacy absolute 64 MiB cap while scanning its supported end-record, central-directory, local-header/name, and package-classification checks through bounded source reads. Legacy discovery also observes cooperative cancellation between completed method stages, while retaining an explicit non-claim for parser-loop interruption and progress reporting. A four-scenario synthetic performance corpus makes sparse, signature-dense, refusal-heavy, and multi-candidate regression behavior visible. Any further source-access work requires a separately designed migration for another method; no whole-scan streaming claim is justified yet.[15] [16] [17] [18] [19] [20]
...
[19]: pdf-window-discovery-v1.md "PDF windowed discovery v1"
[20]: zip-window-discovery-v1.md "ZIP/Open XML windowed discovery v1"
