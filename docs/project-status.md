...
| PNG, JPEG, GIF, and PDF discovery | PNG, JPEG, GIF, and PDF structural discovery use fixed 1 MiB primary source windows, method-specific signature overlap and ownership, bounded file-backed validation, and legacy-candidate parity enforcement. JPEG retains the existing 128 MiB structural cap; GIF and PDF retain their existing 64 MiB structural caps. | The full source is still buffered for filesystem metadata, legacy compatibility discovery, AVI/MP4/ZIP carving, recovery, and exports; this is not a complete streaming-scan or parser-level cancellation claim. |
...
The completed PNG, JPEG, GIF, and PDF increments now have method-specific fixed-window discovery contracts, primary-window ownership, boundary/refusal controls, completed-window cancellation, and legacy candidate parity. JPEG retains the legacy absolute 128 MiB structural cap while scanning entropy data through bounded sequential file reads; GIF retains the legacy absolute 64 MiB cap while scanning its supported block stream through bounded sequential file reads; PDF retains the legacy absolute 64 MiB cap while scanning its supported header, EOF, `startxref`, and `xref` checks through bounded source reads. Legacy discovery also observes cooperative cancellation between completed method stages, while retaining an explicit non-claim for parser-loop interruption and progress reporting. A four-scenario synthetic performance corpus makes sparse, signature-dense, refusal-heavy, and multi-candidate regression behavior visible. Any further source-access work requires a separately designed migration for another method; no whole-scan streaming claim is justified yet.[15] [16] [17] [18] [19]
