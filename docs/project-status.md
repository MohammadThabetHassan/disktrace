...
| PNG and JPEG discovery | PNG and JPEG structural discovery use fixed 1 MiB primary source windows, method-specific signature overlap and ownership, bounded file-backed validation, and legacy-candidate parity enforcement. JPEG retains the existing 128 MiB structural cap. | The full source is still buffered for filesystem metadata, GIF/AVI/MP4/PDF/ZIP carving, recovery, and exports; this is not a complete streaming-scan or parser-level cancellation claim. |
...
| --- | --- | --- |
| Linux desktop | Local bundle and native desktop smoke passed; the exact hosted Ubuntu 24.04 `Verify` workflow passed the completed cancellation-checkpoint matrix.[9] | It is not a signed production installer or a general Linux distribution claim. |
...
| Release governance | `main` requires the exact `Rust workspace and recovery fixtures`, `Windows x86_64 bundle and installer`, `CodeQL Rust analysis`, and `macOS 14 ARM64 workspace validation` contexts; it also requires an up-to-date branch, one current CODEOWNERS review for pull requests, linear history, and resolved conversations, and blocks force pushes and deletion. Exact hosted Linux, Windows, macOS 14 ARM64, and CodeQL workflows passed for `d16d1d32b96a438b8e23095ddad48ebc69c214aa`.[9] [10] [11] [12] | No semantic tag, signed release artifact, GitHub Release, macOS package/manual-acceptance evidence, or release provenance exists. |
...
| Code scanning | The least-privilege Rust CodeQL workflow completed successfully for exact revision `d16d1d32b96a438b8e23095ddad48ebc69c214aa`.[10] | A successful scan is not a security certification, a proof that all vulnerabilities are absent, or a production-release claim. |
...
The completed PNG and JPEG increments now have method-specific fixed-window discovery contracts, primary-window ownership, boundary/refusal controls, completed-window cancellation, and legacy candidate parity. JPEG retains the legacy absolute 128 MiB structural cap while scanning entropy data through bounded sequential file reads. Legacy discovery also observes cooperative cancellation between completed method stages, while retaining an explicit non-claim for parser-loop interruption and progress reporting. A four-scenario synthetic performance corpus makes sparse, signature-dense, refusal-heavy, and multi-candidate regression behavior visible. The next source-access work is a separately designed migration for another method; no whole-scan streaming claim is justified yet.[15] [16] [17]
...
[9]: https://github.com/MohammadThabetHassan/disktrace/actions/runs/32830680530 "Hosted Linux verification for d16d1d3"
[10]: https://github.com/MohammadThabetHassan/disktrace/actions/runs/32830680640 "Hosted CodeQL analysis for d16d1d3"
[11]: https://github.com/MohammadThabetHassan/disktrace/actions/runs/32830680613 "Hosted macOS 14 ARM64 validation for d16d1d3"
[12]: https://github.com/MohammadThabetHassan/disktrace/actions/runs/32830680634 "Hosted Windows distribution, installer acceptance, synthetic corpus, and SBOM review artifact for d16d1d3"
