...
The completed PNG increment now has a fixed-window discovery contract, primary-window ownership, boundary/refusal controls, cancellation after a completed window, and legacy candidate parity. Legacy discovery now also observes cooperative cancellation between completed method stages, while retaining an explicit non-claim for parser-loop interruption and progress reporting. A four-scenario synthetic performance corpus makes sparse, signature-dense, refusal-heavy, and multi-candidate regression behavior visible. The next source-access work is a separately designed migration for another method; no whole-scan streaming claim is justified yet.[15] [16]
