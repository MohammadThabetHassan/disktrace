...
> A passing installer acceptance gate proves one clean hosted runner completed the defined package mechanics and that every installed file listed in the staged SHA-256 manifest matched its expected hash before uninstall. It does **not** launch the GUI, prove visible or correct rendering, test the Start Menu shortcut or `Start DiskTrace.cmd` launcher, certify real-user usability or accessibility, validate SmartScreen or code signing, test upgrades or arbitrary existing machines, establish crash freedom, or create a public release.
...
The hosted Windows workflow builds and validates the project on `windows-2022`, packages the ZIP, compiles the installer with Inno Setup, verifies the ZIP, validates the disposable installed-file SHA-256 manifest and install/uninstall path, calculates installer checksums, and uploads the artifacts for review. It has read-only repository permissions and no release-publishing step. Each result must be tied to its exact source revision; it is not a semantic-versioned or publicly distributed release.
